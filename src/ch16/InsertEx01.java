...
public class InsertEx01 { 
	...
	public static void main(String[] args) {
		try {
			...
			pstmt.setInt(1, 4);													 // ?를 사용하게 해준다
			pstmt.setString(2, "김땡땡");										 // ?를 사용하게 해준다
			pstmt.setString(3, "a2a@nate.com");								    // ?를 사용하게 해준다
			pstmt.setString(4, "1234");										     // ?를 사용하게 해준다
															     				// 파싱을 한 번만 함, 인젝션공격을 막아준다
			pstmt.executeUpdate(); //버퍼에 쓰기(commit이 있음)
			System.out.println("인서트 완료");	//select 는 execute쓰면된다(변화가 없으니까)  
